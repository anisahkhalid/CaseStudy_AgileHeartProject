...
def main():
    st.title("❤️ Heart Disease Prediction App (Old UI V1)")
    st.write("This version collects **feedback before prediction** (included for Agile evidence).")

    model_v1, model_v2 = load_models()

    st.subheader("Patient Inputs")

    # Inputs based on training column names
    ...
    st.divider()
    st.subheader("Step 1: Provide Feedback (Old UI flow)")

    feedback_score = st.slider("Feedback Score (Usability & Clarity)", 1, 5, 3)
    feedback_comment = st.text_area(
        "Feedback Comment (optional)",
        "Confusing flow. Asked to give feedback before seeing prediction."
    )
    ...
    if st.button("Predict with both models"):
        ...
        with col1:
            ...
            st.write("Probability (disease):", round(prob_v1, 4))
        ...
        with col2:
            ...
            st.write("Probability (disease):", round(prob_v2, 4))
            st.write("Latency (ms):", round(latency_v2, 2))

        # Log both predictions using the same feedback
...
if __name__ == "__main__":
    st.set_page_config(page_title="Old UI (UI V1)", layout="wide")
    main()
